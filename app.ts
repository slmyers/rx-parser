...
const pchar = (targetChar: Character, label: string): Parser => {
...
const anyOf = (chars: string) => {
    return chars.split('')
                .map( char => pchar(char, `[pchar: ${char}]`))
...
/* 
    ...
    const pchar_a = pchar('a', `[pchar: a]`);
    const pchar_b = pchar('b', `[pchar: b]`);
    const pchar_c = pchar('c', `[pchar: c]`);
    const pchar_e = pchar('e', `[pchar: e]`);
    usage: 

    const parser = orElse(pchar('b', `[pchar: b]`), pchar('a', `[pchar: a]`), `[orElse: a, b]`);
    Rx.Observable.of(<Response>['', false, '', 'ab']).let(parser).subscribe( val => console.log(val));
    [ '[orElse: a, b]', true, 'a', 'b' ]
    ...
    Rx.Observable.of(<Response>['', false, '', 'ae']).let(a_or_b_andThen_c_or_e).subscribe( val => console.log(val));
    ...
    Rx.Observable.of(<Response>['', false, '', 'blah']).let(stringParser).subscribe( val => console.log(val));
